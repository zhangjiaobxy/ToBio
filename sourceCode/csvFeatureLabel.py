...
if bio == 'b3':
    ...
    bioList = [bio]
for e_bio in bioList:
    if e_bio == 'b1':
        blastFile = open('./../data/rawData/blast.b', 'r')  # blast file
        blast_ls = blastFile.readlines()[1:]
        blast_ls = sorted(blast_ls, key = lambda x:float(x.strip().split('\t')[1]))  # sorted by db column
        blast_ls = sorted(blast_ls, key = lambda x:float(x.strip().split('\t')[0]))  # sorted by query column
        blast_s = []  # blast score list
        for blast_l in blast_ls:
            blast_l = blast_l.strip().split('\t')[2]  # blast score
            blast_s.append(blast_l)
        blastFile.close()
    elif e_bio == 'b2':
        goFile = open('./../data/rawData/go.b', 'r')  # go file
        go_ls = goFile.readlines()[1:]
        go_ls = sorted(go_ls, key = lambda x:float(x.strip().split('\t')[1]))  # sorted by db column
        go_ls = sorted(go_ls, key = lambda x:float(x.strip().split('\t')[0]))  # sorted by query column
        go_s = []  # go score list
        for go_l in go_ls:
            go_l = go_l.strip().split('\t')[2]  # go score
            go_s.append(go_l)
        goFile.close()   
...
for e_subgraphSize in runSize:
    ...
    ffeature=open('./../data/mfinderFeature/featureSize'+e_subgraphSize+'.t','r')
    ...
    if bio == 'b1':
        csvtitle.insert(-1,'blast')
    elif bio == 'b2':
        csvtitle.insert(-1,'go')
    elif bio == 'b3':
        csvtitle.insert(-1,'blast')
        csvtitle.insert(-1,'go')
    ...
    with open('./../data/csvFeatureLabel/featureSize'+e_subgraphSize+'_'+bioScore+'_'+label_file.replace('.txt','')+'.csv', 'wb') as csvfile:
        ...
        while line1:
            if e_subgraphSize=='2':
                u=[float(line1.split()[2]),float(line1.split()[3])]  # subgraph frequency of query network
                v=[float(line1.split()[4]),float(line1.split()[5])]  # subgraph frequency of db network
            ...
            if bio == 'b1':
                floatrow.append(blast_s[line1_iter])  # add blast score to the row
            elif bio == 'b2':
                floatrow.append(go_s[line1_iter])  # add go score to the row
            elif bio == 'b3':                
                floatrow.append(blast_s[line1_iter])  
                floatrow.append(go_s[line1_iter])  
...
if subgraphSize == '234':
    dfs2 = pd.read_csv('./../data/csvFeatureLabel/featureSize2_'+bioScore+'_'+label_file.replace('.txt','.csv'), skipinitialspace=True, delimiter=',')
    dfs3 = pd.read_csv('./../data/csvFeatureLabel/featureSize3_'+bioScore+'_'+label_file.replace('.txt','.csv'), skipinitialspace=True, delimiter=',')
    dfs4 = pd.read_csv('./../data/csvFeatureLabel/featureSize4_'+bioScore+'_'+label_file.replace('.txt','.csv'), skipinitialspace=True, delimiter=',')
    if bio == 'b1' or bio == 'b2':
        features2 = dfs2.columns[:(dfs2.shape[1]-2)]  # remove bio column and label column
        features3 = dfs3.columns[2:(dfs3.shape[1]-2)]
        features4 = dfs4.columns[2:(dfs4.shape[1])]
    elif bio == 'b3':
        features2 = dfs2.columns[:(dfs2.shape[1]-3)]  # remove bio column and label column
        features3 = dfs3.columns[2:(dfs3.shape[1]-3)]
        features4 = dfs4.columns[2:(dfs4.shape[1])]

    dfs234 = pd.concat([dfs2[features2],dfs3[features3],dfs4[features4]],axis=1)
    dfs234.to_csv('./../data/csvFeatureLabel/featureSize234_'+bioScore+'_'+label_file.replace('.txt','.csv'), header=True, index = False)  
