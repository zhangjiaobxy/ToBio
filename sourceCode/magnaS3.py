...
for file_name in file_list:
	if file_name.endswith('_final_stats.txt'):
		if dbStart <= float(file_name.replace('.gw_final_stats.txt','').replace('.gw','').split('-')[-1]) <= dbEnd:
...
for each_name in f_n:
	...
	for each_line in f_original:
		if 'S3_score ' in each_line:
			ec = each_line.strip().split(' ')[-1]
			ec_store.append(ec)

f_generate = open('magna_s3_label' + '.txt', 'w')
...
for i in range(len(ec_store)):
	if i <= len(ec_store):
		final_f_first = f_n[i].replace('.gw_final_stats.txt','').replace('.gw','').strip().split('-')[0]
		final_f_second = f_n[i].replace('.gw_final_stats.txt','').replace('.gw','').strip().split('-')[1]
