...
for e_subgraphSize in runSize:
    ...
    fwdb=open(rootDir+"dbSubgraphSize"+e_subgraphSize+".t",'wb')
    fwquery=open(rootDir+"querySubgraphSize"+e_subgraphSize+".t",'wb')

    list_dirs = os.walk(rootDir)
    for root, dirs, files in list_dirs:
        for f in files:
            if f.endswith('_OUT.txt'):
                # if f[-9] == 2:
    ...
    def readRawData_featrue(fil2read,file2write):
        ...
        for key in dict2write:
            ...
            f.write('\n')
        f.close()
    readRawData_featrue(rootDir+"querySubgraphSize"+e_subgraphSize+".t",rootDir+"querySubgraphSize"+e_subgraphSize+"Feature.t")
    readRawData_featrue(rootDir+"dbSubgraphSize"+e_subgraphSize+".t",rootDir+"dbSubgraphSize"+e_subgraphSize+"Feature.t")
    ...
    def combine_q_db_features(queryfile,netfile,file2write):
        ...
        ftraining.close()
        ftrain.close()
    combine_q_db_features(rootDir+"querySubgraphSize"+e_subgraphSize+"Feature.t",rootDir+"dbSubgraphSize"+e_subgraphSize+"Feature.t",rootDir+'featureSize'+e_subgraphSize+'_temp.t')

########################## sort feature file ##########################################
    featureSort = open(rootDir+'featureSize'+e_subgraphSize+'_temp.t', 'r')
    featureSortWrite = open(rootDir+'featureSize'+e_subgraphSize+'.t', 'w')
    ...
    if os.path.exists(rootDir+'featureSize'+e_subgraphSize+'_temp.t'):  # remove useless file
        os.remove(rootDir+'featureSize'+e_subgraphSize+'_temp.t')
